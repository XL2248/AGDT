...
def _read_score_record(filename):
    ...
    with tf.gfile.GFile(filename) as fd:
        for line in fd:
            name, score, score_hard = line.strip().split(":")
            name = name.strip()[1:-1]
            score = float(score)
            score_hard = float(score_hard)
            records.append([name, score, score_hard])
...
def _save_score_record(filename, records):
    ...
    with tf.gfile.GFile(filename, "w") as fd:
        for record in sorted_records:
            checkpoint_name, score, score_hard = record
            fd.write("\"%s\": %f: %f\n" % (checkpoint_name, score, score_hard))
...
def _add_to_record(records, record, max_to_keep):
    ...
    for (name, score, score_hard) in records:
        models[name] = score
...
def _evaluate(eval_fn, input_fn, decode_fn, path, config, params):
    graph = tf.Graph()
    with graph.as_default():
        features = input_fn()
        refs = features["target"]
        flag = features["flag"]
        logits = eval_fn(features)
        tgt_vocab_size = len(params.vocabulary["target"])

        logit = tf.argmax(tf.nn.softmax(logits),dimension = 1)
        results = {
            "predictions": logit,
            "references": refs,
            "flag": flag
        ...
        rig = 0
        total = 0
        hard_right = 0 # correct hard number
        hard_total = 0 # total hard number

        with tf.train.MonitoredSession(session_creator=sess_creator) as sess:
            while not sess.should_stop():
                outputs = sess.run(results)
                predictions = outputs["predictions"].tolist()
                references = [item[0] for item in outputs["references"]]
                flag = [item[0] for item in outputs["flag"]]
                if len(predictions) != len(references):
                    print("error of len(predictions) and len(references)")
                for i in range(len(predictions)):
                    if predictions[i] == references[i]:
                        rig += 1
                    if int(flag[i]) == 1: # hard set
                        hard_total += 1
                        if predictions[i] == references[i]:
                            hard_right += 1
                total += len(predictions)
        
        return rig/(total*1.0), hard_right*1.0/hard_total
...
class EvaluationHook(tf.train.SessionRunHook):
    ...
    def __init__(self, eval_fn, eval_input_fn, eval_decode_fn, base_dir,
                 session_config, params, eval_secs=None,
                 eval_steps=None, metric="F1-Score"):
        ...
        if metric != "F1-Score":
            raise ValueError("Currently, EvaluationHook only support F1-Score")
        ...
        self._max_to_keep = params.keep_top_checkpoint_max
        self._metric = metric
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._timer.should_trigger_for_step(stale_global_step + 1):
            ...
            if self._timer.should_trigger_for_step(global_step):
                ...
                score, score_hard = _evaluate(self._eval_fn, self._eval_input_fn,
                                  self._eval_decode_fn,
                                  self._base_dir,
                                  self._session_config,
                                  self._params)
                tf.logging.info("%s at step %d: test score: %f. test hard score:%f" %
                                (self._metric, global_step, score, score_hard))
                ...
                record = [latest_checkpoint, score, score_hard]
                ...
                hard_score = records[0][2]
            
                if best_score > self._best_s:
                    self._best_s = best_score
                    self._step = global_step
                tf.logging.info("Best score at step %d: %f: %f" %
                                (self._step, self._best_s, hard_score))
    ...
    def end(self, session):
        ...
        if last_step != self._timer.last_triggered_step():
            ...
            best_score = records[0][1]
            hard_score = records[0][2]
            tf.logging.info("Best score: %f %f" %( best_score, hard_score))
